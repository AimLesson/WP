    ...
    <div class="content-wrapper">
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-12">
                        <a href="{{ route('setup.createsotarget') }}" class="btn btn-primary mb-3"><i
                                class="fas fa-plus"></i>
                            Add</a>
                        <div class="card">
                            <div class="card-header">
                            <h3 class="card-title">SO Target</h3>
                        </div>
                            <!-- /.card-header -->
                            <div class="card-body">
                                <table id="machine" class="table table-head-fixed text-nowrap">
                                    ...
                                    <tbody>
                                        @php
                                            ...
                                            $sotarget = \App\Models\sotarget::get();
                                        @endphp
                                        @foreach ($sotarget as $st)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td>{{ $st->customer }}</td>
                                                <td>{{ $st->product_type }}</td>
                                                <td>{{ $st->total_order }}</td>
                                                <td>{{ $st->nominal_value }}</td>
                                                <td>{{ $st->year }}</td>
                                                <td>
                                                    <a href="{{ route('setup.editsotarget', ['id' => $st->id]) }}"
                                                        class="btn-xs btn-warning"><i class="fas fa-pen"></i>
                                                        Edit</a>
                                                    <a href="" data-toggle="modal"
                                                        data-target="#modal-hapus{{ $st->id }}"
                                                        class="btn-xs btn-danger"><i class="fas fa-trash-alt"></i>
                                                        Hapus</a>
                                                </td>
                                            </tr>
                                            <div class="modal fade" id="modal-hapus{{ $st->id }}">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h4 class="modal-title">Confirm Delete SO Target</h4>
                                                        ...
                                                        <div class="modal-footer justify-content-between">
                                                            <form
                                                                action="{{ route('setup.deletesotarget', ['id' => $st->id]) }}"
                                                                method="POST">
                                                                @csrf
                                                                @method('DELETE')
