    ...
    <div class="content-wrapper">
       ...
       <section class="content">
        <div class="container-fluid">
            <div class="row">
                <div class="col-12">
                    <a href="{{ route('setup.createshipping') }}" class="btn btn-primary mb-3"><i class="fas fa-plus"></i>
                        Add</a>
                    <div class="card">
                        <div class="card-body">
                            <table id="machine" class="table table-head-fixed text-nowrap">
                                ...
                                <tbody>
                                    @foreach ($shipping as $sg)
                                        <tr>
                                            <td>{{$sg->id_shipping}}</td>
                                            <td>{{$sg->shipping_name}}</td>
                                            <td>
                                                <a href="{{ route('setup.editshipping', ['id' => $sg->id]) }}"
                                                    class="btn-xs btn-warning"><i class="fas fa-pen"></i>
                                                    Edit</a>
                                                <a href="" data-toggle="modal"
                                                    data-target="#modal-hapus{{ $sg->id }}"
                                                    class="btn-xs btn-danger"><i class="fas fa-trash-alt"></i>
                                                    Hapus</a>
                                            </td>
                                        </tr>
                                        <div class="modal fade" id="modal-hapus{{ $sg->id }}">
                                            <div class="modal-dialog">
                                                <div class="modal-content">
                                                    <div class="modal-header">
                                                        <h4 class="modal-title">Confirm Delete Shipping</h4>
                                                    ...
                                                    <div class="modal-body">
                                                        <p>Are you sure delete
                                                            <b>{{ $sg->id_shipping }} - {{ $sg->shipping_name }}?</b>
                                                        </p>
                                                    </div>
                                                    <div class="modal-footer justify-content-between">
                                                        <form action="{{ route('setup.deleteshipping', ['id' => $sg->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            @method('DELETE')
